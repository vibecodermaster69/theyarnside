...
export default function CartDrawer() {
  ...
  const mailto = useMemo(() => `mailto:cozy@theyarnside.com?subject=${encodeURIComponent("New THE YARN SIDE order")}&body=${encodeURIComponent([`Name: ${form.name}`, `Phone: ${form.phone}`, `Address: ${address}`, "", ...items.map((item) => `${item.name} x ${item.quantity}`)].join("\n"))}`, [address, form, items]);
  ...
  async function submitOrder(event: FormEvent) {
    ...
    try {
      ...
      const { error } = await createSupabaseBrowserClient().rpc("create_order", { p_customer_name: form.name, p_customer_email: form.email || null, p_customer_phone: form.phone, p_delivery_address: address, p_notes: form.notes || (giftWrap ? "Gift wrapping requested (+₹30)." : null), p_policy_version: POLICY_VERSION, p_policy_accepted_at: policyAcceptedAt, p_items: items.map((item) => ({ product_id: item.id, quantity: item.quantity })) });
  ...
  return <div className="cart-layer" role="dialog" aria-modal="true" aria-label="Shopping cart"><button className="cart-backdrop" onClick={closeCart} aria-label="Close cart" /><aside className="cart-panel">
    ...
    </form> : <><div className="cart-items">{items.map((item) => <div className="cart-item" key={item.id}><img src={item.image} alt="" /><div className="cart-item-info"><h3>{item.name}</h3><p>{formatInr(item.priceInr)}</p><div className="quantity-controls"><button type="button" onClick={() => updateQuantity(item.id, item.quantity - 1)}><Minus size={14} /></button><span>{item.quantity}</span><button type="button" onClick={() => updateQuantity(item.id, item.quantity + 1)}><Plus size={14} /></button></div></div><button type="button" className="remove-item" onClick={() => removeItem(item.id)}><Trash2 size={16} /></button></div>)}</div><div className="cart-summary"><span>Total</span><strong>{formatInr(totalInr)}</strong></div><button className="btn btn-primary checkout-button" onClick={() => setCheckoutOpen(true)}>Continue to checkout</button><button className="clear-button" onClick={clearCart}>Clear cart</button></>}
