                      ...
                      <div className="form-card" style={{ marginTop: "16px", padding: "16px", background: "var(--color-brand-light)" }}>
                        ...
                        {(draft.color_variants ?? []).map((variant, index) => (
                          <div className="colour-variant-row" key={`colour-variant-${index}`} style={{ display: "grid", gridTemplateColumns: "1fr 52px .7fr 1.4fr auto", gap: "8px", alignItems: "end", marginTop: "12px" }}>
                            ...
                            <label className="colour-image-field">Colour image<select value={variant.imageUrl ?? ""} onChange={(e) => { const next = [...draft.color_variants]; next[index] = { ...variant, imageUrl: e.target.value }; setDraft({ ...draft, color_variants: next }); }}><option value="">Use default image</option>{(draft.image_urls ?? []).map((url, imageIndex) => <option key={url} value={url}>Image {imageIndex + 1}</option>)}</select>{(variant.imageUrl || draft.image_url || draft.image_urls?.[0]) && <img className="colour-image-preview" src={variant.imageUrl || draft.image_url || draft.image_urls?.[0] || ""} alt={`${variant.name || "Colour"} preview`} />}</label>
